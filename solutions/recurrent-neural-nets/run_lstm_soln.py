...
if __name__ == '__main__':
    ...
    if sine:
        ...
        normalize = False
        lstm_structure = {'first_layer_units': 50, 
                          'second_layer_units': 75,
                          'dense_layer_units': 1}
    ...
    X_train, y_train, X_test, y_test = lstm_model.load_data(filename, lstm_structure['first_layer_units'], normalize)
    ...
    if sine:
        ...
        predicted_full = lstm_model.predict_sequence_full(X_test, lstm_structure['first_layer_units'])
        plot_results(predicted_full, y_test, 'Sine_wave-predict_full_sequence_from_start_seed')
    else:
        predictions = lstm_model.predict_sequences_multiple(X_test, lstm_structure['first_layer_units'], lstm_structure['first_layer_units'])
        plot_results_multiple(predictions, y_test, lstm_structure['first_layer_units'], 'Stock_prediction')
